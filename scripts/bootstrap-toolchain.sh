...
#
# It writes two files that are gitignored: scripts/env.sh, which the other scripts source, and
# local.properties, which points AGP at the SDK.
#
...
# Generated by scripts/bootstrap-toolchain.sh. Gitignored. Source it, or use scripts/build.sh.
...
# This JDK is a relocated Ubuntu package and its default trust-store lookup finds nothing, so
# every HTTPS fetch fails with "the trustAnchors parameter must be non-empty".
